...
<c:forEach items="${requestScope.orderVoList}" var="orderVo" varStatus="status">
    <div style="width: 60%; margin-left: 20%; margin-right: 20%; height: 306px; border: 2px solid black; margin-top: 5%; background-color: honeydew; position: relative">
        ...
        <c:if test="${orderVo.wsOrderStatus == 0}">
            ...
            <a href="${pageContext.request.contextPath}/fore/seller/order/confirm-delivery?id=${orderVo.wsOrderId}" style="text-decoration: none"><button class="btn-danger" style="position: absolute; z-index: 2; right: 2%; bottom: 5%">confirm delivery</button></a>
        ...
        <c:if test="${orderVo.wsOrderStatus == 2 || orderVo.wsOrderStatus == 3}">
            ...
            <a href="${pageContext.request.contextPath}/fore/seller/order/score-for-trade?id=${orderVo.wsOrderId}" style="text-decoration: none"><button class="btn-danger" style="position: absolute; z-index: 2; right: 2%; bottom: 5%">rate for trade</button></a>
            <%--</c:if><a href="" style="text-decoration: none">--%>
            <%--<button class="btn-danger" style="position: absolute; z-index: 2; right: 2%; top: 5%;">Date:</button>--%>
        </c:if>

        <c:if test="${orderVo.wsOrderStatus == 4}">
            <%--==4 意味着seller评价完了，要等待buyer评价 --%>
            <span style="position: absolute; z-index: 2; right: 2%; top: 5%;color: blue">wait for buyer</span>
            <%--</c:if><a href="" style="text-decoration: none">--%>
            <%--<button class="btn-danger" style="position: absolute; z-index: 2; right: 2%; top: 5%;">Date:</button>--%>
        </c:if>
